...
<link rel="stylesheet" href="{{ asset('css/create-categories.css') }}">
    ...
    <div>
        <center><h2><ion-icon name="apps-outline"></ion-icon> Edit Cliente Type</h2><br><br></center>
        <form class="form"action="{{ route('tipocliente.update', $tipo->id) }}" method="POST">
            @csrf
            @method("PUT")
            <div class="form-group">
                <label><ion-icon name="copy-outline"></ion-icon> Cliente Type</label>
                <input type="text" name="description" class="form-control" placeholder="Category" required value="{{ $tipo->description }}">
            ...
            <a class="btn btn-danger" href="{{ route('tipocliente.index') }}" role="button"><ion-icon name="return-down-back-outline"></ion-icon> Go Back</a>
            </center>
        </form>
        @if ($errors ->any())
